...
If FindString(response, ~"\"isError\":false", 1) = 0 Or FileSize(allowedRoot + "probe-export.csv") <= 0
  PrintN("sqlite/export csv probe failed")
  End 1
EndIf

response = JSONRPC_Dispatcher_Dispatch(@dispatcher, 0, ~"{\"jsonrpc\":\"2.0\",\"method\":\"tools/call\",\"params\":{\"name\":\"sqlite/export\",\"arguments\":{\"dbPath\":\"probe.sqlite\",\"sql\":\"SELECT id, locale, title FROM admin_notes ORDER BY id\",\"outputPath\":\"probe-export.ods\",\"format\":\"ods\",\"maxRows\":10,\"overwrite\":true}},\"id\":5}")
If FindString(response, ~"\"isError\":false", 1) = 0 Or FileSize(allowedRoot + "probe-export.ods") <= 0
  PrintN("sqlite/export ods probe failed")
  End 1
EndIf
